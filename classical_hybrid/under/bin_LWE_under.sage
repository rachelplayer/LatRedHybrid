...
def find_r_bin_LWE_under(n, q, m, k_vals = [4], min_delta = 1.002, max_delta = 1.03):
	global time_map
	time_map = {}
	...
	for r in k_vals:
		...
		def f(x):
			f_x = RR(bkzcosts_one_round(dim,x) - log(nr_loops_bin(r,c_0,dim, det, x, Y, q, size_S)*rt_NP_under(dim) / (p_c * prob_NP(dim, det, Y, x, q)), 2))
			return f_x
